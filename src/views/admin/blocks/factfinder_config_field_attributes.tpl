[{if $module_var === 'ffExportAttributes'}]
    ...
    <attribute-rows headers='Name, Multi-Attribute' selected-attributes='[{$selectedAttributes|@json_encode}]' available-attributes='[{$availableAttributes|@json_encode}]'></attribute-rows>
[{else}]
  [{$smarty.block.parent}]
[{/if}]
